...
Includes the mandatory tenant-isolation check for the views added here (ADR-0002).
"""
...
@pytest.mark.django_db
def test_signup_duplicate_email_race_is_the_same_generic_message(client):
    """Two signups pass the form checks; the DB constraint loses the second one."""
...
@pytest.mark.parametrize(
    "next_url",
    ["https://evil.example/x", "//evil.example/x", "http://testserver.evil.example/", "javascript:alert(1)"],
)
def test_login_ignores_unsafe_next(client, user, next_url):
    resp = client.post("/login/", {"email": user.email, "password": GOOD_PW, "next": next_url})
    assert resp["Location"] == "/trades/"
    client.post("/logout/")
    resp = client.post("/login/", {"email": user.email, "password": GOOD_PW}, QUERY_STRING=f"next={next_url}")
    assert resp["Location"] == "/trades/"


@pytest.mark.django_db
def test_login_bad_credentials_are_identical_for_unknown_email_wrong_password_and_inactive(client, user):
    inactive = User.objects.create_user(email="off@example.com", password=GOOD_PW, is_active=False)

    def attempt(email, password):
        resp = client.post("/login/", {"email": email, "password": password})
        return resp.status_code, resp.context["form"].errors.as_data(), resp.context["form"].non_field_errors()

    unknown = attempt("nobody@example.com", GOOD_PW)
    wrong = attempt(user.email, "wrong-password-1")
    off = attempt(inactive.email, GOOD_PW)

    assert unknown[0] == wrong[0] == off[0] == 200
    assert unknown[2] == wrong[2] == off[2] == [copy.LOGIN_BAD_CREDENTIALS]
    assert list(unknown[1]) == list(wrong[1]) == list(off[1]) == ["__all__"]  # no field-level hint
...
# --- logout, root, protected pages -------------------------------------------------------


@pytest.mark.django_db
def test_logout_is_post_only(client, user):
    client.force_login(user)
...
@pytest.mark.django_db
def test_trades_page_is_not_cached(client, user):
    client.force_login(user)
    resp = client.get("/trades/")
    assert resp.status_code == 200
...
# --- tenant isolation (ADR-0002, mandatory) ----------------------------------------------


@pytest.mark.django_db
def test_user_b_sees_none_of_user_a_through_the_views_added_here(client):
    from django.test import Client

    from journal.tests import _make_execution, _make_import_batch

    a = User.objects.create_user(email="alice-private@example.com", password=GOOD_PW, timezone="Asia/Tokyo")
    b = User.objects.create_user(email="bob@example.com", password=GOOD_PW)
    _make_import_batch(a)
    _make_execution(a)
    ...
    resp = client.get("/trades/")
    assert resp.status_code == 200
    assert resp.wsgi_request.user == b
    assert b"alice-private@example.com" not in resp.content
    assert b"bob@example.com" in resp.content
